    ...
    Private Sub WithMovementSubject(stuffToDo As Action(Of IDirection, Mock(Of IWorldData), ICharacterMovement))
        WithSubject(
            Sub(worldData, id, subject)
                ...
                worldData.Verify(Function(x) x.StatisticType.ReadDefaultValue(firstStatisticId))
                worldData.Verify(Function(x) x.StatisticType.ReadDefaultValue(secondStatisticId))
                worldData.Verify(Function(x) x.StatisticType.ReadDefaultValue(thirdStatisticId))
                worldData.Verify(Function(x) x.Character.ReadLocation(id))
                worldData.Verify(Function(x) x.Player.Read())
            End Sub)
    End Sub
