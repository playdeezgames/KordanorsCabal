...
Public MustInherit Class ModeProcessor
    ...
    End Sub
    Protected Shared Sub ShowFacing(buffer As PatternBuffer, xy As (Integer, Integer), player As IPlayerCharacter)
        buffer.WriteText(xy, $"Facing: {player.Movement.Direction.Name}", False, Hue.Black)
    End Sub
    Protected Shared Sub ShowExits(buffer As PatternBuffer, xy As (Integer, Integer), player As IPlayerCharacter)
    ...
    Friend MustOverride Sub UpdateBuffer(player As IPlayerCharacter, buffer As PatternBuffer)
    Friend MustOverride Sub UpdateButtons(player As IPlayerCharacter)
    Friend MustOverride Function HandleButton(player As IPlayerCharacter, button As Button) As UIState
    Friend MustOverride Function HandleRed(player As IPlayerCharacter) As UIState
    ...
    Friend Shared Sub PopButtonIndex()
        ...
        CurrentButtonIndex = 0
    End Sub
    Protected Sub ShowDungeon(buffer As PatternBuffer, player As IPlayerCharacter)
        ...
        If location.Routes.Exists(Direction.FromId(StaticWorldData.WorldData, 5L)) Then
        ...
        If location.Routes.Exists(Direction.FromId(StaticWorldData.WorldData, 6L)) Then
        ...
        If location.Routes.Exists(Direction.FromId(StaticWorldData.WorldData, 8L)) Then
            ShowSprite(buffer, (5, 5), location.Routes.Find(Direction.FromId(StaticWorldData.WorldData, 8L)).RouteType.Sprite)
        End If

        If location.Routes.Exists(Direction.FromId(StaticWorldData.WorldData, 7L)) Then
            ShowSprite(buffer, (5, 5), location.Routes.Find(Direction.FromId(StaticWorldData.WorldData, 7L)).RouteType.Sprite)
        End If
