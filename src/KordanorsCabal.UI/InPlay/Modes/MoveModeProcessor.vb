...
Friend Class MoveModeProcessor
    ...
    Friend Overrides Sub UpdateButtons(player As IPlayerCharacter)
        ...
        End If
        If player.Movement.CanMove(Direction.FromId(WorldData, 5L)) Then
            Buttons(UpButtonIndex).Title = "Up"
        End If
        If player.Movement.CanMove(Direction.FromId(WorldData, 6L)) Then
            Buttons(DownButtonIndex).Title = "Down"
        End If
        If player.Movement.CanMove(Direction.FromId(WorldData, 7L)) Then
            Buttons(InButtonIndex).Title = "In"
        End If
        If player.Movement.CanMove(Direction.FromId(WorldData, 8L)) Then
    ...
    Friend Overrides Function HandleButton(player As IPlayerCharacter, button As Button) As UIState
        Select Case button.Index
            ...
            Case DownButtonIndex
                Return HandleMove(player, Direction.FromId(WorldData, 6L))
            Case UpButtonIndex
                Return HandleMove(player, Direction.FromId(WorldData, 5L))
            Case InButtonIndex
                Return HandleMove(player, Direction.FromId(WorldData, 7L))
            Case OutButtonIndex
                Return HandleMove(player, Direction.FromId(WorldData, 8L))
    ...
    Private Function HandleMove(player As IPlayerCharacter, direction As IDirection) As UIState
        If player.Movement.CanMove(direction) Then
            ...
            If player.Movement.Move(direction) Then
                player.EnqueueMessage(Nothing, "You take damage from starvation!")
