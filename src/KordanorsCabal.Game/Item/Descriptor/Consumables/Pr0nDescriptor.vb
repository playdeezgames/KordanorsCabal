    ...
    Sub New()
        MyBase.New(
            ...
            Sub(character)
                ...
                character.ChangeStatistic(CharacterStatisticType.FromId(CharacterStatisticTypeUtility.Stress), -healRoll)
                Dim lines As New List(Of String)
                lines.Add($"You make use of {ItemType.Pr0n.Name}, which cheers you up by {healRoll} {CharacterStatisticType.FromId(MP).Name}.")
                lines.Add($"You now have {character.CurrentMP} {CharacterStatisticType.FromId(MP).Name}.")
                Dim lotionItem = character.Inventory.ItemsOfType(ItemType.Lotion).FirstOrDefault
                If lotionItem Is Nothing Then
                    lines.Add($"You also receive 10 {CharacterStatisticType.FromId(CharacterStatisticTypeUtility.Chafing).Name}. Try {ItemType.Lotion.Name} next time.")
                    character.Chafing = 10
                Else
                    lines.Add($"You use a bit of {ItemType.Lotion.Name} to prevent {CharacterStatisticType.FromId(CharacterStatisticTypeUtility.Chafing).Name}.")
