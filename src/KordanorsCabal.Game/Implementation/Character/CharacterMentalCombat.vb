Public Class CharacterMentalCombat
    ...
    Function RollInfluence() As Long Implements ICharacterMentalCombat.RollInfluence
        Return RollDice(If(Character.Statistics.GetStatistic(StatisticType.FromId(WorldData, StatisticTypeInfluence)), 0) + NegativeInfluence())
    End Function
    Function RollWillpower() As Long Implements ICharacterMentalCombat.RollWillpower
        Return RollDice(If(Character.Statistics.GetStatistic(StatisticType.FromId(WorldData, StatisticTypeWillpower)), 0) + NegativeInfluence())
    End Function
    ReadOnly Property IsDemoralized() As Boolean Implements ICharacterMentalCombat.IsDemoralized
        Get
            If Character.Statistics.GetStatistic(StatisticType.FromId(WorldData, StatisticTypeWillpower)).HasValue Then
                Return CurrentMP <= 0
            End If
            Return False
        End Get
    ...
    Sub AddStress(delta As Long) Implements ICharacterMentalCombat.AddStress
        Character.Statistics.ChangeStatistic(StatisticType.FromId(WorldData, StatisticTypeStress), delta)
    End Sub
    ReadOnly Property CanIntimidate As Boolean Implements ICharacterMentalCombat.CanIntimidate
        Get
            If Not Character.Statistics.GetStatistic(StatisticType.FromId(WorldData, StatisticTypeWillpower)).HasValue Then
                Return False
            End If
    ...
    Property CurrentMP As Long Implements ICharacterMentalCombat.CurrentMP
        Get
            Return Math.Max(0, Character.Statistics.GetStatistic(StatisticType.FromId(WorldData, StatisticTypeMP)).Value - Character.Statistics.GetStatistic(StatisticType.FromId(WorldData, StatisticTypeStress)).Value)
        End Get
        Set(value As Long)
            Character.Statistics.SetStatistic(StatisticType.FromId(WorldData, StatisticTypeStress), Character.Statistics.GetStatistic(StatisticType.FromId(WorldData, StatisticTypeMP)).Value - value)
        End Set
    End Property
    ReadOnly Property CanDoIntimidation() As Boolean Implements ICharacterMentalCombat.CanDoIntimidation
        Get
            If If(Character.Statistics.GetStatistic(StatisticType.FromId(WorldData, StatisticTypeInfluence)), 0) <= 0 Then
                Return False
            End If
