Public Class Renderer
    ReadOnly Property FrameBuffer As FrameBuffer
    ...
    Sub New(borderSize As (Integer, Integer), gridSize As (Integer, Integer), cellSize As (Integer, Integer))
        BorderWidth = borderSize.Item1
        BorderHeight = borderSize.Item2
        CellWidth = cellSize.Item1
        CellHeight = cellSize.Item2
        FrameBuffer = New FrameBuffer(borderSize.Item1 * 2 + gridSize.Item1 * cellSize.Item1, borderSize.Item2 * 2 + gridSize.Item2 * cellSize.Item2)
        ...
        For y = 0 To FrameBuffer.Rows - 1
            ...
            For x = 0 To FrameBuffer.Columns - 1
                If x < BorderWidth OrElse y < BorderHeight OrElse x >= FrameBuffer.Columns - BorderWidth OrElse y >= FrameBuffer.Rows - BorderHeight Then
                    FrameBuffer.Pixel(x, y) = HueColors(BorderHue)
                    Continue For
                End If
                ...
                FrameBuffer.Pixel(x, y) = HueColors(hue)
            Next
        Next
    End Sub
